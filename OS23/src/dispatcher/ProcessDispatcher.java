...
public class ProcessDispatcher extends TimerTask {
	...
	public void run() { //Dispatcher baþlýyor.
		...
		for (InterruptedProcess interruptedProcess : InterruptedProcesses) 
		//Askýyla alýnmýþ kuyruklar dönülüyor ve 20 saniye aþýmýna uðrayýp uðramadýklarý kontrol ediliyor.
		{
			if(timer - interruptedProcess.InterruptedTime == 20)
			{
				//20 saniyeyi aþan prosesler "TERMINATED" durumuna çekiliyor.
				switch(interruptedProcess.type)
				{
				case USERBASED2:
				{
					for(int i = 0; i < UserBasedProcess2.getSize(); i++)
					{
						if(interruptedProcess.PID == UserBasedProcess2.getir(i).PID)
						{
							try {
								Process p = UserBasedProcess2.getir(i);
								p.ProcessSituation = ProcessSituation.TERMINATED;
								String t ="proses zaman asimina ugradi."+"(id:"+p.PID+"    oncelik: "+p.Priority+"  kalan sure: "+p.BurstTime +" sn)";
								p.ExecuteProcess(p, t);	
								//UserBasedProcess2.degistir(i, p);
								//ChangeSituationOfProcess(i, ProcessSituation.TERMINATED, ProcessType.USERBASED2);
							} catch (IOException e) {
				...
				case USERBASED3:
				{
					for(int i = 0; i < UserBasedProcess3.getSize(); i++)
					{
						if(interruptedProcess.PID == UserBasedProcess3.getir(i).PID)
						{
							try {
								ChangeSituationOfProcess(i, ProcessSituation.TERMINATED, ProcessType.USERBASED3);
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
		...
		//Askýda olmasý hasebiyle "TERMINATED" durumuna cekilmiþ fakat kuyruktan silinmemiþ prosesler var ise siliniyor.
		if(UserBasedProcess2.getir(UserBasedProcess2.ilk) != null && UserBasedProcess2.getir(UserBasedProcess2.ilk).ProcessSituation == ProcessSituation.TERMINATED)
			UserBasedProcess2.cikar();
		
		if(UserBasedProcess3.getir(UserBasedProcess3.ilk) != null && UserBasedProcess3.getir(UserBasedProcess3.ilk).ProcessSituation == ProcessSituation.TERMINATED)
	...
	{			
		...
		{				
			...
			InterruptedProcesses.add(new InterruptedProcess(p.PID, timer, ProcessType.USERBASED2));
			p.Priority=2;
			UserBasedProcess1.cikar();
		...
		{				
			...
			if(!InterruptControl(p.PID))
				InterruptedProcesses.add(new InterruptedProcess(p.PID, timer, ProcessType.USERBASED3));
			p.Priority=3;
			UserBasedProcess2.cikar();
	...
	private boolean InterruptControl(String pid)
	{
		for (InterruptedProcess interruptedProcess : InterruptedProcesses) {
			if(interruptedProcess.PID == pid) {
				interruptedProcess.InterruptedTime = timer;
				return true;
			}
